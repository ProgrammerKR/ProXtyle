...
document.querySelectorAll('#nav-tab>[data-bs-toggle="tab"]').forEach(el => {
  el.addEventListener('shown.bs.tab', () => {
    ...
    ProXtyle.ScrollSpy.getOrCreateInstance(scrollElem).refresh()
  })
})
```
...
```js
const scrollSpy = new ProXtyle.ScrollSpy(document.body, {
...
dataSpyList.forEach(dataSpyEl => {
  ProXtyle.ScrollSpy.getInstance(dataSpyEl).refresh()
})
```
