...
- Tooltips rely on the third party library [Popper](https://popper.js.org/docs/v2/) for positioning. You must include [popper.min.js]({{< param "cdn.popper" >}}) before `ProXtyle.js`, or use one `ProXtyle.bundle.min.js` which contains Popper.
...
const tooltipList = [...tooltipTriggerList].map(tooltipTriggerEl => new ProXtyle.Tooltip(tooltipTriggerEl))
```
...
const tooltip = new ProXtyle.Tooltip(exampleEl, options)
```
...
```js
const tooltip = new ProXtyle.Tooltip('#example', {
...
```js
const tooltip = new ProXtyle.Tooltip(element, {
...
```js
const tooltip = ProXtyle.Tooltip.getInstance('#example') // Returns a Bootstrap tooltip instance
...
const tooltip = ProXtyle.Tooltip.getOrCreateInstance(myTooltipEl)
