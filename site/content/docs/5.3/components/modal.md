...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@getProXtyle">Open modal for @getProXtyle</button>
...
Embedding YouTube videos in modals requires additional JavaScript not in Bootstrap to automatically stop playback and more. [See this helpful Stack Overflow post](https://stackoverflow.com/questions/18622508/ProXtyle-3-and-youtube-in-modal) for more information.
...
```js
const myModal = new ProXtyle.Modal(document.getElementById('myModal'), options)
// or
const myModalAlternative = new ProXtyle.Modal('#myModal', options)
```
...
```js
const myModal = new ProXtyle.Modal('#myModal', {
  keyboard: false
})
```
