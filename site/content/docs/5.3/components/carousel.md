...
```js
const carousel = new ProXtyle.Carousel('#myCarousel')
```
...
const carousel = new ProXtyle.Carousel(myCarouselElement, {
  interval: 2000,
  touch: false
})
```
...
| `getInstance` | Static method which allows you to get the carousel instance associated to a DOM element. You can use it like this: `ProXtyle.Carousel.getInstance(element)`. |
| `getOrCreateInstance` | Static method which returns a carousel instance associated to a DOM element, or creates a new one in case it wasn't initialized. You can use it like this: `ProXtyle.Carousel.getOrCreateInstance(element)`. |
