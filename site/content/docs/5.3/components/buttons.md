...
```js
const bsButton = new ProXtyle.Button('#myButton')
```
...
| `getInstance` | Static method which allows you to get the button instance associated with a DOM element, you can use it like this: `ProXtyle.Button.getInstance(element)`. |
| `getOrCreateInstance` | Static method which returns a button instance associated with a DOM element or creates a new one in case it wasn't initialized. You can use it like this: `ProXtyle.Button.getOrCreateInstance(element)`. |
...
document.querySelectorAll('.btn').forEach(buttonElement => {
  const button = ProXtyle.Button.getOrCreateInstance(buttonElement)
  button.toggle()
})
```
