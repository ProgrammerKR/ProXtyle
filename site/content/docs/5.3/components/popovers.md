...
- Popovers rely on the third party library [Popper](https://popper.js.org/docs/v2/) for positioning. You must include [popper.min.js]({{< param "cdn.popper" >}}) before `ProXtyle.js`, or use one `ProXtyle.bundle.min.js` which contains Popper.
...
const popoverList = [...popoverTriggerList].map(popoverTriggerEl => new ProXtyle.Popover(popoverTriggerEl))
```
...
```js
const popover = new ProXtyle.Popover('.example-popover', {
  container: 'body'
})
```
...
```js
const popover = new ProXtyle.Popover('.popover-dismiss', {
  trigger: 'focus'
})
```
...
const popover = new ProXtyle.Popover(exampleEl, options)
```
...
```js
const popover = new ProXtyle.Popover(element, {
...
const popover = ProXtyle.Popover.getOrCreateInstance('#example') // Returns a Bootstrap popover instance
