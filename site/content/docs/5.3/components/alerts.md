...
const alerts = [...alertList].map(element => new ProXtyle.Alert(element))
```
...
```js
const bsAlert = new ProXtyle.Alert('#myAlert')
```
...
| `getInstance` | Static method which allows you to get the alert instance associated to a DOM element. For example: `ProXtyle.Alert.getInstance(alert)`. |
| `getOrCreateInstance` | Static method which returns an alert instance associated to a DOM element or create a new one in case it wasn't initialized. You can use it like this: `ProXtyle.Alert.getOrCreateInstance(element)`. |
{{< /bs-table >}}
