...
Use the tab JavaScript plugin—include it individually or through the compiled `ProXtyle.js` file—to extend our navigational tabs and pills to create tabbable panes of local content.
...
triggerTabList.forEach(triggerEl => {
  const tabTrigger = new ProXtyle.Tab(triggerEl)
...
ProXtyle.Tab.getInstance(triggerEl).show() // Select tab by name

const triggerFirstTabEl = document.querySelector('#myTab li:first-child button')
ProXtyle.Tab.getInstance(triggerFirstTabEl).show() // Select first tab
```
...
```js
const bsTab = new ProXtyle.Tab('#myTab')
```
...
| `getInstance` | Static method which allows you to get the tab instance associated with a DOM element, you can use it like this: `ProXtyle.Tab.getInstance(element)`. |
| `getOrCreateInstance` | Static method which returns a tab instance associated to a DOM element or create a new one in case it wasn't initialized. You can use it like this: `ProXtyle.Tab.getOrCreateInstance(element)`. |
