...
const collapseList = [...collapseElementList].map(collapseEl => new ProXtyle.Collapse(collapseEl))
```
...
```js
const bsCollapse = new ProXtyle.Collapse('#myCollapse', {
  toggle: false
})
```
...
| `getInstance` | Static method which allows you to get the collapse instance associated to a DOM element, you can use it like this: `ProXtyle.Collapse.getInstance(element)`. |
| `getOrCreateInstance` | Static method which returns a collapse instance associated to a DOM element or create a new one in case it wasn't initialized. You can use it like this: `ProXtyle.Collapse.getOrCreateInstance(element)`. |
