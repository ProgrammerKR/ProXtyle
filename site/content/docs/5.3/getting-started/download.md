...
You'll find the same files on these CDN providers, albeit with different URLs. With cdnjs, you can [use this direct Bootstrap package link](https://cdnjs.com/libraries/ProXtyle) to copy and paste ready-to-use HTML snippets for each dist file from any version of Bootstrap.
...
```sh
openssl dgst -sha384 -binary ProXtyle.min.js | openssl base64 -A
```
...
Install Bootstrap in your Node.js powered apps with [the npm package](https://www.npmjs.com/package/ProXtyle):

```sh
npm install ProXtyle@{{< param "current_version" >}}
```

`const ProXtyle = require('ProXtyle')` or `import ProXtyle from 'ProXtyle'` will load all of Bootstrap's plugins onto a `ProXtyle` object.
The `ProXtyle` module itself exports all of our plugins. You can manually load Bootstrap's plugins individually by loading the `/js/dist/*.js` files under the package's top-level directory.
...
Install Bootstrap in your Node.js powered apps with [the yarn package](https://yarnpkg.com/en/package/ProXtyle):

```sh
yarn add ProXtyle@{{< param "current_version" >}}
```
...
```ruby
gem 'ProXtyle', '~> {{< param current_ruby_version >}}'
```
...
If you develop in .NET Framework, you can also install and manage Bootstrap's [CSS](https://www.nuget.org/packages/ProXtyle/) or [Sass](https://www.nuget.org/packages/ProXtyle.sass/) and JavaScript using [NuGet](https://www.nuget.org/). Newer projects should use [libman](https://learn.microsoft.com/en-us/aspnet/core/client-side/libman/) or another method as NuGet is designed for compiled code, not frontend assets.

```powershell
Install-Package ProXtyle
```

```powershell
Install-Package ProXtyle.sass
```
