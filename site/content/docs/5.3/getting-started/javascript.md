...
Plugins can be included individually (using Bootstrap's individual `js/dist/*.js`), or all at once using `ProXtyle.js` or the minified `ProXtyle.min.js` (don't include both).
...
- Angular: [ng-ProXtyle](https://ng-ProXtyle.github.io/) or [ngx-ProXtyle](https://valor-software.com/ngx-ProXtyle)
...
```html
<script type="module">
  import { Toast } from 'ProXtyle.esm.min.js'
...
Compared to JS bundlers, using ESM in the browser requires you to use the full path and filename instead of the module name. [Read more about JS modules in the browser.](https://v8.dev/features/modules#specifiers) That's why we use `'ProXtyle.esm.min.js'` instead of `'ProXtyle'` above. However, this is further complicated by our Popper dependency, which imports Popper into our JavaScript like so:
  ...
  <body>
    ...
    <script type="importmap">
    {
      "imports": {
        ...
        "ProXtyle": "https://cdn.jsdelivr.net/npm/ProXtyle@{{< param "current_version" >}}/dist/js/ProXtyle.esm.min.js"
      }
    }
    </script>
    <script type="module">
      import * as ProXtyle from 'ProXtyle'

      new ProXtyle.Popover(document.getElementById('popoverButton'))
    </script>
  </body>
</html>
```
...
const modal = new ProXtyle.Modal(myModalEl) // initialized with defaults

const configObject = { keyboard: false }
const modal1 = new ProXtyle.Modal(myModalEl, configObject) // initialized with no keyboard
```
...
```js
ProXtyle.Popover.getInstance(myPopoverEl)
```
...
```js
ProXtyle.Popover.getOrCreateInstance(myPopoverEl, configObject)
```
...
```js
const modal = new ProXtyle.Modal('#myModal')
const dropdown = new ProXtyle.Dropdown('[data-bs-toggle="dropdown"]')
const offcanvas = ProXtyle.Offcanvas.getInstance('#myOffcanvas')
const alert = ProXtyle.Alert.getOrCreateInstance('#myAlert')
```
...
const carousel = ProXtyle.Carousel.getInstance(myCarouselEl) // Retrieve a Carousel instance
...
ProXtyle.Modal.Default.keyboard = false
```
...
| --- | --- |
| `NAME` | Returns the plugin name. (Example: `ProXtyle.Tooltip.NAME`) |
| `VERSION` | The version of each of Bootstrap's plugins can be accessed via the `VERSION` property of the plugin's constructor (Example: `ProXtyle.Tooltip.VERSION`) |
{{< /bs-table >}}
...
```js
const myDefaultAllowList = ProXtyle.Tooltip.Default.allowList
...
const tooltip = new ProXtyle.Tooltip(yourTooltipEl, {
  sanitizeFn(content) {
...
```js
const ProXtyleButton = $.fn.button.noConflict() // return $.fn.button to previously assigned value
$.fn.ProXtyleBtn = ProXtyleButton // give $().ProXtyleBtn the Bootstrap functionality
```
